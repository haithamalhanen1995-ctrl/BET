...
import { Star, Zap, ShoppingBag, CheckCircle, RefreshCw, AlertTriangle, ArrowRight, Calendar } from "lucide-react";
...
export const TaskView: React.FC = () => {
  const { language, currentUser, rateProduct, vipTiers, products, settings } = useApp();
  const t = translations[language];
  const isRtl = language === "ar";

  const isTodayHoliday = !!(
    settings.isHolidayEnabled &&
    settings.holidayDays &&
    settings.holidayDays.includes(new Date().getDay())
  );
  ...
  const handleGrabTask = () => {
    if (isTodayHoliday) {
      alert(language === "ar" ? "اليوم عطلة أسبوعية! لا يمكن تنفيذ مهام." : "Today is a weekend holiday! Tasks are disabled.");
      return;
    }
  ...
  const handleConfirmRating = async () => {
    ...
    setTimeout(async () => {
      const result = await rateProduct(selectedProduct.id);
      ...
      <div className="bg-gradient-to-r from-slate-900 to-slate-950 p-4 rounded-2xl border border-slate-800 shadow-lg flex justify-between items-center">
        <div>
          <span className="text-[10px] uppercase font-bold text-amber-500 block">
            {vipTiers.find(v => v.level === currentUser.vipLevel)?.name || (currentUser.vipLevel === 0 ? "VIP 0" : `G${currentUser.vipLevel}`)} ROOM
          </span>
        ...
        <AnimatePresence mode="wait">
          {currentUser.vipLevel === 0 ? (
            ...
            >
              ...
              <h3 className="text-sm font-bold text-white">
                {language === "ar" ? "الغرفة مغلقة! عطلة نهاية الأسبوع" : "Room Closed! Weekend Holiday"}
              </h3>
              <p className="text-xs text-slate-400 max-w-xs leading-relaxed">
                {language === "ar"
                  ? "اليوم هو عطلة رسمية بالمنصة. لا يمكن تنفيذ المهام التلقائية اليوم. يمكنك الاستمرار في سحب وإيداع الأموال بشكل طبيعي دون تأثر."
                  : "Today is a scheduled platform holiday. Daily tasks are suspended. Withdrawals and deposits remain fully active and unaffected."}
              </p>
              <div className="bg-slate-950 px-4 py-2.5 rounded-xl border border-slate-850 mt-2 text-xs text-amber-500 font-bold">
                {language === "ar" ? "تمنياتنا لكم بعطلة سعيدة" : "Have a great weekend!"}
              </div>
            </motion.div>
          ) : isLoading ? (
